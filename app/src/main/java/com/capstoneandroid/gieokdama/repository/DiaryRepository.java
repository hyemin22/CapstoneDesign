...
import com.capstoneandroid.gieokdama.api.DiaryApiService;
...
public class DiaryRepository {
    ...
    // 앨범명 수정
    public void updateAlbumName(Long albumId, String albumName, DiaryRepository.DiaryCallback callback) {
        Call<Void> call = diaryApiService.updateAlbumName(albumId, albumName);
        call.enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    // 서버로부터 성공 응답을 받았을 때 처리
                    System.out.println("앨범명 수정 성공");
                    callback.onSuccess(); // 성공 시 콜백 호출
                } else {
                    // 서버 응답이 있지만 오류가 있을 때 처리
                    System.out.println("앨범명 수정 실패: " + response.errorBody());
                    callback.onFailure("서버 오류: " + response.message());
                }
            }
    ...
    // 앨범 삭제
    public void deleteAlbum(Long albumId, DiaryRepository.DiaryCallback callback) {
        Call<Void> call = diaryApiService.deleteAlbum(albumId);
        call.enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    ...
                    // 서버 응답이 있지만 오류가 있을 때 처리
                    System.out.println("앨범 삭제 실패: " + response.errorBody());
                    callback.onFailure("서버 오류: " + response.message());
                }
            }

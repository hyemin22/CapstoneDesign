...
import com.capstoneandroid.gieokdama.model.WishList;
...
public class WishListRepository {
    ...
    // 위시리스트 삭제
    public void deleteWishDataToServer(Long id, WishListRepository.WishListCallback callback) {
        // 위시리스트 데이터 DELETE 요청
        Call<Void> call = wishListApiService.deleteWish(id);
        ...
        call.enqueue(new Callback<Void>() {
            ...
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    ...
                    // 서버 응답이 있지만 오류가 있을 때 처리
                    System.out.println("위시리스트 삭제 실패: " + response.errorBody());
                    callback.onFailure("서버 오류: " + response.message());
                }
            }
    ...
    // 좋아요 삭제
    public void deleteLike(Long wishId, Long userId, WishListRepository.WishListCallback callback) {
        Call<Void> call = wishListApiService.deleteLike(wishId, userId);
        call.enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    // 서버로부터 성공 응답을 받았을 때 처리
                    System.out.println("위시 좋아요 삭제 성공");
                    callback.onSuccess(); // 성공 시 콜백 호출
                } else {
                    // 서버 응답이 있지만 오류가 있을 때 처리
                    System.out.println("위시 좋아요 삭제 실패: " + response.errorBody());
                    callback.onFailure("서버 오류: " + response.message());
                }
            }
            @Override

...
public class WishListRepository {
    ...
    public void getFamilyExpectedWishList(Long familyId, WishListRepository.GetListCallback callback) {
        // 인스턴스의 wishListApiService 통해 호출
        // get요청
        Call<List<WishExpectedItem>> call = wishListApiService.getFamilyWishListExpected(familyId);
    ...
    public void sendWishListCategoryToServer(WishList wishList, WishListRepository.WishListCallback callback) {
        //Retrofit 클라이언트 생성, api 서비스 생성
        WishListApiService wishListApiService = RetrofitClient.getClient("http://172.19.8.222:8080").create(WishListApiService.class);

        //위시리스트 데이터 post 요청
        Call<Void> call = wishListApiService.saveWishListCategory(wishList);
    ...
    public void getWishListByCategory(Long userId, WishListRepository.GetListCallback callback) {
        // 인스턴스의 wishListApiService를 통해 호출
        // get요청
        Call<List<WishExpectedItem>> call = wishListApiService.getFamilyWishListCategory(userId);
